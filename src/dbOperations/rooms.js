...
class RoomsTable {
  ...
  async getJoinableRoom(userId, native, language) {
    ...
    let roomsFromDb = await this.dbRoom.find({"language1": language, "language2" : native, "userId2": null, "numberInRoom": {"$eq": 1 }});
    ...
    if (roomsFromDb.length == 0) {
      roomsFromDb = await this.dbRoom.find({"language2": language, "language1" : native, "userId1": null, "numberInRoom": {"$eq": 1 } });
    }
